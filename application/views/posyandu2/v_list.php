...
function edit_posyandu2(id) {
  window.location = _base_url + 'admin/c_posyandu2/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'admin/c_posyandu2/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("admin/c_posyandu2/delete/");?>",
					   data: "items="+itemlist,
