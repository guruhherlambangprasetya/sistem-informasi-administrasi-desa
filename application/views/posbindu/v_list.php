...
function edit_posbindu(id) {
  window.location = _base_url + 'admin/c_posbindu/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'admin/c_posbindu/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("admin/c_posbindu/delete/");?>",
					   data: "items="+itemlist,
