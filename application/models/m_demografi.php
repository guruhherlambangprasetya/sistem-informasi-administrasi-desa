...
class M_demografi extends CI_Model {
  ...
  function __construct(){
	...
	{
		$this->db->select('ref_rt.nomor_rt as rt,count(tbl_keluarga.id_rt) as luas_wilayah');
		
		$sub = $this->subquery->start_subquery('select');
		$sub->select ('count(*)')->from('tbl_keluarga');		
		$sub->where('tbl_penduduk.id_jen_kel','1');
		$sub->where('tbl_keluarga.id_rt = ref_rt.nomor_rt');
		...
		$this->db->join('ref_rt','ref_rt.nomor_rt = tbl_keluarga.id_rt','right');
		$this->db->where('ref_rt.nomor_rt !=','0');
		$this->db->group_by("ref_rt.nomor_rt");
