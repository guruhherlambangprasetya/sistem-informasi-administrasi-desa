...
class C_gsi extends CI_Controller {
    ...
    function lists() 
	{
        ...
        $grid_js = build_grid_js('flex1',site_url('admin/c_gsi/load_data'),$colModel,'id_struktur','asc',$gridParams,$buttons);
		
		$data['js_grid'] = $grid_js;

        $data['page_title'] = 'STRUKTUR KADER GSI';		
		$data['menu'] = $this->load->view('menu/v_admin', $data, TRUE);
        $data['content'] = $this->load->view('gsi/v_list', $data, TRUE);
     ...
     function load_data() 
	{
		...
		$records = $this->m_gsi->get_Gsi_flexigrid();
		...
		foreach ($records['records']->result() as $row)
		{
			$record_items[] = array(
				...
				$row->judul_struktur,
				$row->isi_struktur,		
			'<button type="submit" class="btn btn-default btn-xs" title="Edit" onclick="edit_gsi(\''.$row->id_struktur.'\')"/>
    ...
    function add()
    {
		...
		if($this->session->userdata('logged_in') AND $role == 'Administrator')
		{
			...
			$data['page_title'] = 'Tambah Struktur Kader Gsi';
			$data['menu'] = $this->load->view('menu/v_admin', $data, TRUE);
			$data['content'] = $this->load->view('gsi/v_tambah', $data, TRUE);		
    ...
    function simpan_gsi() {
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			$this->m_gsi->insert_gsi($datagsi);
				
			redirect('admin/c_gsi','refresh');
        }
		else $this->add();
    }
   ...
   function edit($id){
		...
		if($this->session->userdata('logged_in') AND $role == 'Administrator')
		{
			...
			$data['page_title'] = 'UBAH STRUKTUR KADER GSI';
			$data['hasil'] = $this->m_gsi->getGsiByIdStruktur($id);
			$data['menu'] = $this->load->view('menu/v_admin', $data, TRUE);
			$data['content'] = $this->load->view('gsi/v_ubah', $data, TRUE);
    ...
    function update_gsi()
    {
		...
		$this->m_gsi->update_gsi(array('id_struktur' => $id_struktur), $dataGsi);
		redirect('admin/c_gsi','refresh');
		}
