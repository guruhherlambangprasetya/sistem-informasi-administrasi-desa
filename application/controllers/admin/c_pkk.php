...
class C_pkk extends CI_Controller {
    ...
    function lists() 
	{
        ...
        $grid_js = build_grid_js('flex1',site_url('admin/c_pkk/load_data'),$colModel,'id_struktur','asc',$gridParams,$buttons);
		
		$data['js_grid'] = $grid_js;

        $data['page_title'] = 'STRUKTUR KADER PKK ';		
		$data['menu'] = $this->load->view('menu/v_admin', $data, TRUE);
        $data['content'] = $this->load->view('pkk/v_list', $data, TRUE);
     ...
     function load_data() 
	{
		...
		$records = $this->m_pkk->get_pkk_flexigrid();
		...
		foreach ($records['records']->result() as $row)
		{
			$record_items[] = array(
				...
				$row->judul_struktur,
				$row->isi_struktur,		
			'<button type="submit" class="btn btn-default btn-xs" title="Edit" onclick="edit_pkk(\''.$row->id_struktur.'\')"/>
    ...
    function add()
    {
		...
		if($this->session->userdata('logged_in') AND $role == 'Administrator')
		{
			...
			$data['page_title'] = 'Tambah Struktur Kader PKK';
			$data['menu'] = $this->load->view('menu/v_admin', $data, TRUE);
			$data['content'] = $this->load->view('pkk/v_tambah', $data, TRUE);		
    ...
    function simpan_pkk() {
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			$this->m_pkk->insert_pkk($datapkk);
				
			redirect('admin/c_pkk','refresh');
        }
		else $this->add();
    }
   ...
   function edit($id){
		...
		if($this->session->userdata('logged_in') AND $role == 'Administrator')
		{
			...
			$data['page_title'] = 'UBAH STRUKTUR KADER PKK';
			$data['hasil'] = $this->m_pkk->getPkkByIdStruktur($id);
			$data['menu'] = $this->load->view('menu/v_admin', $data, TRUE);
			$data['content'] = $this->load->view('pkk/v_ubah', $data, TRUE);
    ...
    function update_pkk()
    {
		...
		$gambar = $this->m_pkk->getPkkByIdStruktur($id_struktur);
		...
		$this->m_pkk->update_pkk(array('id_struktur' => $id_struktur), $dataPkk);
		redirect('admin/c_pkk','refresh');
		}
